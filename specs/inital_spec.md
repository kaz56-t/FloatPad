...
| 項目 | 採用技術 | 理由 |
|------|----------|------|
| フレームワーク | **Electron** (Node.js + Chromium) | macOS / Windows 両対応。HTML/CSS/JS でUI構築可能。Webビューアも内蔵WebViewで実現 |
| UI | HTML + CSS + Vanilla JS (または React) | 軽量、シンプル |
| ファイル保存 | Node.js `fs` モジュール | ローカルファイルへの読み書き |
| パッケージング | `electron-builder` | Mac (.dmg / .app) / Windows (.exe インストーラー) を出力 |
